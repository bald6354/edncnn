function [X, Y, samples] = events2FeatML_Fastv2(aedat, inputVar)
...
for sampleLoop = 1:numel(sampleList)
    ...
    addEventIdx = ...
        (abs(aedat.data.polarity.y - chipRow) <= inputVar.neighborhood) & ...
        (abs(aedat.data.polarity.x - chipCol) <= inputVar.neighborhood) & ...
...
%     newPos = cell2mat(accumarray([aedat.data.polarity.y(p) aedat.data.polarity.x(p)],aedat.data.polarity.timeStamp(p)-currentSampleTime,aedat.data.frame.size,@(x) {sum(exp(x./decayWeights),1)},{zeros(1,1,numel(decayWeights))}));
    newPosHist = cell2mat(accumarray([aedat.data.polarity.y(p)-chipRow+inputVar.neighborhood+1 aedat.data.polarity.x(p)-chipCol+inputVar.neighborhood+1], currentSampleTime-aedat.data.polarity.timeStamp(p), [size(X,1) size(X,2)], @(x) {mink(cat(3,reshape(x,1,1,[]),inf(1,1,inputVar.depth)), inputVar.depth)},{inf(1,1,inputVar.depth)}));
    p = addEventIdx & aedat.data.polarity.polarity<=0;
%     newNeg = cell2mat(accumarray([aedat.data.polarity.y(p) aedat.data.polarity.x(p)],aedat.data.polarity.timeStamp(p)-currentSampleTime,aedat.data.frame.size,@(x) {sum(exp(x./decayWeights),1)},{zeros(1,1,numel(decayWeights))}));
    newNegHist = cell2mat(accumarray([aedat.data.polarity.y(p)-chipRow+inputVar.neighborhood+1 aedat.data.polarity.x(p)-chipCol+inputVar.neighborhood+1], currentSampleTime-aedat.data.polarity.timeStamp(p), [size(X,1) size(X,2)], @(x) {mink(cat(3,reshape(x,1,1,[]),inf(1,1,inputVar.depth)), inputVar.depth)},{inf(1,1,inputVar.depth)}));
...
%     cChip = chipCol-inputVar.neighborhood:chipCol+inputVar.neighborhood;
    ...
    X(:,:,:,1,sampleLoop) = cat(3, newPosHist, newNegHist);
